...
export function StartHabitBottomNav() {
  ...
  const [animate, setAnimate] = useState('h-3/4 w-1/4 text-mainColor ');
  const bookMarkHandle = () => {
    ...
    if (isBookMark === false) {
      ...
      setAnimate('h-3/4 w-1/4 text-mainColor ');
    }
  ...
  return (
    <div className="flex bg-white h-[3rem] px-6  w-full fixed bottom-0 min-w[300px] justify-center items-center">
      {isBookMark === false ? (
        <AiOutlineHeart
          className="h-3/4 w-1/4 text-mainColor"
      ...
      <button
        className="bg-mainColor h-4/5 w-3/4 rounded-lg ml-5 text-textColor"
